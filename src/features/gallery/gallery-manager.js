class Gallery {
  ...
  async init() {
    // nsfwDetector.init() 已在 app.js 中调用，避免重复初始化
    this.bindEvents();
  ...
  async loadImages(filters = {}) {
    ...
    // 直接从 filterManager 获取最新筛选状态（更可靠）
    const currentFilters = (typeof filterManager !== 'undefined' && filterManager)
      ? { ...filterManager.getFilters(), ...filters }
      : filters;

    console.log(`[Gallery] Loading images: page=${this.currentPage}, limit=${this.limit}, filters=`, currentFilters);
  ...
  renderSkeleton() {
    const count = this.limit || 12;
    const items = Array(count).fill(0);
    return `
      <div class="gallery__grid">
        ${items.map(() => `
          <div class="card gallery-item skeleton">
            <div class="card__image skeleton-img"></div>
  ...
  _getImageSrc(image, isNsfwHidden) {
    return window.resolveApiPath(image.thumbnail || image.path);
  }

  renderGridItem(image, index) {
    const isNsfw = this._isNSFWImage(image);
    const nsfwHidden = isNsfw && !nsfwDetector.shouldDisplay();
    const imgSrc = window.resolveApiPath(image.thumbnail || image.path);
    return `
  ...
  async renameImage(path, newName) {
    return new Promise((resolve, reject) => {
      ...
      xhr.open('POST', (window.API_BASE_URL || '/api') + '/rename', true);
  ...
  setupLazyLoading() {
    ...
    const imageObserver = new IntersectionObserver((entries, observer) => {
      ...
      rootMargin: '300px 300px', // 增加预加载范围，上下左右各300px
      threshold: 0.01
    });
  ...
  animateImages() {
    const items = document.querySelectorAll('.gallery-item');
    const maxDelay = 300; // 最大延迟300ms，避免后面的图片等待太久
    
    items.forEach((item, index) => {
      item.style.opacity = '0';
      item.style.transform = 'translateY(20px)';
      // 延迟随索引增加，但不超过最大值
      const delay = Math.min(index * 30, maxDelay);
