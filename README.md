...
>
> 核心思路：在 `extensions-builtin/sd-gallery/scripts/sd\_gallery.py` 里用 `script\_callbacks.on\_ui\_tabs` 注册一个 iframe Tab，再用 `on\_app\_started` 把 Flask app 通过 `WSGIMiddleware` 挂到 FastAPI 上。前端不用改，后端直接把 `OUTPUTS\_DIR` 指向 WebUI 的 outputs 目录就好。
>
> 大概长这样：
> ```python
> from modules import script\_callbacks
> from sd\_gallery.backend.app import app as flask\_app
> from fastapi.middleware.wsgi import WSGIMiddleware
>
> def on\_app\_started(demo, app):
>     app.mount("/sd-gallery", WSGIMiddleware(flask\_app))
>
> def on\_ui\_tabs():
...
>
> script\_callbacks.on\_app\_started(on\_app\_started)
> script\_callbacks.on\_ui\_tabs(on\_ui\_tabs)
> ```
>
...
本项目已按照标准的前端项目结构进行重构，代码组织清晰，易于维护和扩展。

```
sd-gallery/
├── backend/                 # 后端Python代码
│   ├── routes/              # API路由
│   │   ├── images.py        # 图片相关API（列表、下载、重命名、删除等）
│   │   ├── trash.py         # 回收站相关API（恢复、删除、清空等）
│   │   └── nsfw.py          # NSFW关键词管理API
│   │   └── \_\_init\_\_.py      # 路由包初始化
│   ├── services/            # 业务逻辑服务
│   │   ├── image\_service.py # 图片处理服务（扫描、元数据提取、缩略图生成）
│   │   ├── trash\_service.py # 回收站管理服务
│   │   ├── nsfw\_service.py  # NSFW关键词管理服务
│   │   └── \_\_init\_\_.py      # 服务包初始化
│   ├── utils/               # 工具函数
│   │   ├── metadata.py      # 元数据提取工具
│   │   ├── cache.py         # 缓存管理工具
│   │   ├── path.py          # 路径处理工具
│   │   └── \_\_init\_\_.py      # 工具包初始化
│   ├── app.py               # Flask应用主入口
│   └── \_\_init\_\_.py          # 后端包初始化
│
├── src/                     # 前端代码
│   ├── components/          # 可复用UI组件
│   │   ├── lightbox.js      # 图片预览灯箱组件
│   │   ├── modal.js         # 模态框组件
│   │   └── pager.js         # 分页器组件
│   ├── features/            # 业务功能模块
│   │   ├── gallery/         # 图片画廊功能
│   │   │   ├── gallery-manager.js  # 画廊管理器
│   │   │   └── use-filter.js       # 筛选逻辑Hook
│   │   ├── model/           # 模型筛选功能
│   │   │   └── model-filter.js     # 模型筛选管理器
│   │   ├── lora/            # LoRA 筛选功能
│   │   │   └── lora-filter.js      # LoRA 筛选管理器（含名称映射）
│   │   ├── nsfw/            # NSFW检测功能
│   │   │   └── nsfw-detector.js    # NSFW检测器
│   │   ├── settings/        # 设置功能
│   │   │   └── settings-manager.js # 设置管理器
│   │   └── trash/           # 回收站功能
│   │   │   └── trash-manager.js    # 回收站管理器
│   ├── hooks/               # 通用业务逻辑
│   │   ├── use-responsive.js # 响应式布局Hook
│   │   └── use-theme.js      # 主题切换Hook
│   ├── pages/               # 页面入口
│   │   └── gallery-page.js  # 画廊页面入口
│   ├── services/            # API客户端
...
├── PROJECT\_STRUCTURE.md     # 项目结构说明文档
├── FILE\_NAVIGATION.md       # 文件导航说明文档
├── .gitignore               # Git忽略文件配置
└── README.md                # 本文档
```
...
* **backend/**: 后端Python代码，按功能模块分离

  * **routes/**: API路由定义，处理HTTP请求
  * **services/**: 业务逻辑实现，封装核心功能
  * **utils/**: 工具函数，提供基础功能支持
  * **app.py**: Flask应用主入口
* **src/**: 前端代码，遵循标准前端项目结构

  * **components/**: 可复用的UI组件（原子/分子级别）
  * **features/**: 业务功能模块，按功能拆分
  * **hooks/**: 跨模块的可复用状态/副作用逻辑
  * **pages/**: 页面入口文件
...
* 智能提取 Prompt 中的 LoRA/LyCORIS 标签（`<lora:name>` / `<lyco:name>`），用于 LoRA 分类筛选
* 若无参数，相应字段留空，不影响浏览
...
|**时间范围**|全部 / 今天 / 本周 / 本月|
|**模型分类**|动态发现所有使用的模型，智能合并版本变体|
|**LoRA 分类**|从提示词 `<lora:xxx>` 标签中动态提取，支持名称映射|
...
* **LoRA 名称映射**：自定义 LoRA 在筛选列表中的显示名称，方便识别 `<lora:xxx>` 标签对应的实际用途
...
```bash
   git clone https://github.com/LaoBiDeng321/SD\_Gallery.git
   cd SD\_Gallery
   ```
...
3. **确认 `outputs` 目录路径**
默认情况下，`backend/app.py` 会寻找 `../outputs`（即项目父目录下的 outputs）。如果你的 SD WebUI 不在同级目录，可以修改 `backend/app.py` 中的 `OUTPUTS\_DIR` 变量：

```python
   OUTPUTS\_DIR = Path(\_\_file\_\_).parent.parent / 'outputs'  # 默认配置
   # 或修改为绝对路径：
   OUTPUTS\_DIR = Path(r"D:\\stable-diffusion-webui\\outputs")   # Windows 示例
   # 或
   OUTPUTS\_DIR = Path("/home/user/stable-diffusion-webui/outputs")  # Linux 示例
   ```

4. **启动服务**：

   * Windows: 双击 `run.bat`
   * Linux/Mac: 在终端中执行 `bash run.sh`
   * 直接运行: 执行 `python -m backend.app`

   启动成功后终端会显示：

   ```
   ============================================================
   SD Gallery - AI Image Gallery System
   ============================================================

   Checking Python environment...
   ✓ Python environment normal

   Outputs directory: D:\\stable-diffusion-webui\\outputs
...
* 当你通过 SD WebUI 生成了新图片后，刷新页面即可，服务端会自动检测到变化并更新
  ...
  主要配置位于 `backend/app.py` 文件顶部，可按需修改：

|变量名|默认值|说明|
|-|-|-|
|`OUTPUTS\_DIR`|`../outputs`|SD 输出目录的绝对或相对路径|
|`TRASH\_DIR`|`../outputs/.trash`|回收站目录|
|`NSFW\_KEYWORDS\_FILE`|`../nsfw\_keywords.json`|NSFW关键词文件路径|
|`THUMBS\_DIR`|`../.thumbs`|缩略图缓存目录|

详细配置和业务逻辑参数位于对应的service文件中：

* 图片处理配置: `backend/services/image\_service.py`（最大文件大小、缩略图尺寸等）
* 缓存配置: `backend/utils/cache.py`（缓存持续时间）
